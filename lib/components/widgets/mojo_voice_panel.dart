...
class MojoVoicePanel extends StatefulWidget {
  ...
  const MojoVoicePanel({super.key, required this.service, required this.onClose, this.onTranscript});
...
class _MojoVoicePanelState extends State<MojoVoicePanel> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.black.withAlpha(51), blurRadius: 20, offset: const Offset(0, 4))],
      ),
      child: Column(
  ...
  Widget _buildHeader() {
    return Row(
      children: [
        Icon(CupertinoIcons.waveform, color: AppColors.getThemeTextColor(context), size: 18),
        const SizedBox(width: 8),
        Text(
          'MoJo Voice',
          style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.getThemeTextColor(context)),
        ),
        const Spacer(),
        GestureDetector(
          onTap: widget.onClose,
          child: Icon(CupertinoIcons.xmark_circle_fill, color: AppColors.getInactiveTextColor(context), size: 22),
        ),
      ],
    );
  }
    ...
    switch (_state) {
            ...
            AnimatedBuilder(
              ...
              builder: (context, child) {
                return Container(
                  width: 12,
                  height: 12,
                  decoration: BoxDecoration(color: Colors.red.withAlpha((_pulseAnimation.value * 255).toInt()), shape: BoxShape.circle),
                );
              },
            ),
            const SizedBox(width: 8),
      ...
      case MojoVoiceState.processing:
        return Row(
          ...
          children: [
            ...
            Text(
              'Processing',
              style: TextStyle(fontSize: 13, color: AppColors.getThemeTextColor(context), fontWeight: FontWeight.w500),
            ),
          ],
        );
      case MojoVoiceState.playing:
        return Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(CupertinoIcons.speaker_2_fill, color: Theme.of(context).colorScheme.primary, size: 14),
            const SizedBox(width: 8),
            Text(
              'Playing reply',
      ...
      case MojoVoiceState.idle:
        return Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(CupertinoIcons.checkmark_circle, color: Colors.green, size: 14),
            const SizedBox(width: 8),
            Text(
              'Ready',
  ...
  Widget _buildDuration() {
    ...
    return Text(
      ...
      style: TextStyle(fontSize: 24, fontFamily: 'monospace', fontWeight: FontWeight.w300, color: AppColors.getThemeTextColor(context)),
    );
  }
  ...
  Widget _buildStatusText() {
    ...
    return Text(text, style: TextStyle(fontSize: 11, color: AppColors.getInactiveTextColor(context)));
  }
}
...
class _WaveformPainter extends CustomPainter {
  ...
  _WaveformPainter({required this.state, required this.progress, required this.animationValue, required this.textColor, required this.accentColor});
  ...
  bool shouldRepaint(covariant _WaveformPainter oldDelegate) {
    return oldDelegate.state != state || oldDelegate.progress != progress || oldDelegate.animationValue != animationValue;
  }
}
...
class MojoVoicePanelOverlay {
  ...
  static void show({required BuildContext context, required MojoVoiceService service, void Function(String transcript)? onTranscript}) {
    hide();
    ...
    _currentEntry = OverlayEntry(
      builder: (context) => Positioned(
        ...
        child: Material(
          ...
          child: MojoVoicePanel(service: service, onClose: hide, onTranscript: onTranscript),
        ),
      ),
    );
