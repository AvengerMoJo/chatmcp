...
class ConversationSummarizer {
  ...
  static Future<ConversationSummary> summarize({
    ...
    if (messages.isEmpty) {
      return ConversationSummary(summary: '', originalMessageCount: 0, originalTokenCount: 0, summaryTokenCount: 0);
    }
  ...
  static String buildContextWithSummary({required List<ChatMessage> recentMessages, required ConversationSummary summary, String? systemPrompt}) {
    final buffer = StringBuffer();
  ...
  static List<ChatMessage> buildCompressedMessages({
    ...
    if (summary.summary.isNotEmpty) {
      result.add(ChatMessage(role: MessageRole.system, content: '[Context summary of earlier conversation]: ${summary.summary}'));
    }
