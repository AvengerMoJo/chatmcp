...
class MiMoTtsAdapter implements TtsAdapter {
  ...
  Future<Uint8List?> _requestNonStreamingAudioWavBytes(String spokenText) async {
    ...
    Uint8List? audio = await _requestNonStreamingWithRoles(spokenText, styleRole: 'user', textRole: 'assistant');
    ...
    audio = await _requestNonStreamingWithRoles(spokenText, styleRole: 'system', textRole: 'user');
    return audio;
  }

  Future<Uint8List?> _requestStreamingAudioWavBytes(String spokenText) async {
    // First try: style as user + input as assistant (MiMo TTS-compatible baseline)
    Uint8List? audio = await _requestStreamingWithRoles(spokenText, styleRole: 'user', textRole: 'assistant');
    if (audio != null && audio.isNotEmpty) return audio;

    // Fallback: style as system + input as user (experimental)
    _log.warning('MiMo TTS streaming fallback to experimental roles');
    audio = await _requestStreamingWithRoles(spokenText, styleRole: 'system', textRole: 'user');
    return audio;
  }

  Future<Uint8List?> _requestNonStreamingWithRoles(String spokenText, {required String styleRole, required String textRole}) async {
  ...
  Future<Uint8List?> _requestStreamingWithRoles(String spokenText, {required String styleRole, required String textRole}) async {
    final messages = <Map<String, String>>[];
    if (stylePrompt.isNotEmpty) {
      messages.add({'role': styleRole, 'content': stylePrompt});
    }
